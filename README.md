...
* [`src/hello_world_rf/`](src/hello_world_rf/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/hello_world_rf/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `hello-world-rf` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/hello_world_rf/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns hello-world-rf.example
  (:require [hello-world-rf.config :as config])
